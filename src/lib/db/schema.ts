...
import { blob, customType, index, integer, sqliteTable, text, uniqueIndex } from "drizzle-orm/sqlite-core";
import { DEFAULT_ROLE, type TRole } from "../roles";

// sqlite has no case-insensitive text type - the collation has to ride along in the column
// type, so lookups and the unique index below both ignore case
const textNoCase = customType<{ data: string }>({ dataType: () => "text COLLATE NOCASE" });

export const users = sqliteTable(
  "users",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    username: textNoCase("username").notNull(),
    ...
    accentColor: text("accent_color").notNull().default("#00ffc1"),
    syntaxTheme: text("syntax_theme").notNull().default("github-dark"),
    previewMode: text("preview_mode").notNull().default("panel"),
